...
public class TW_MultiStrings_RandomPointer : MonoBehaviour {
    ...
    private void NewLineCheck(string S)
    {
        if (S.Contains("\n"))
        {
            MakeRandomTypewriterTextWithNewLine(S, MakeList(S));
        }
        else
        {
            MakeRandomTypewriterText(S);
        }
    }

    private void MakeRandomTypewriterText(string ORIGINAL)
    {
        start = false;
    ...
    private void MakeRandomTypewriterTextWithNewLine(string ORIGINAL, List<int> List)
    {
        start = false;
        if (сharIndex != ORIGINAL.Length + 1)
        {
